...
# --- 1. 網頁基本設定 ---
st.set_page_config(layout="wide", page_title="金虎南-區間監控系統")
    ...
    for item in all_sids_info:
        try:
            ...
            if len(download_list) > 1:
                if sid_full in all_data:
                    stock = all_data[sid_full].copy()
            else:
                stock = all_data.copy()

            # 備援機制：如果批量抓不到或資料全為空，嘗試單獨下載該檔
            if stock.empty or stock['Close'].isnull().all():
                stock = yf.download(sid_full, period="120d", progress=False)

            # 處理 MultiIndex 並確保資料欄位存在
            if isinstance(stock.columns, pd.MultiIndex): stock.columns = stock.columns.get_level_values(0)
            if stock.empty or 'Close' not in stock.columns or len(stock) < 5: continue
...
if "data" not in st.session_state:
    with st.spinner('連線資料庫中...'): st.session_state["data"] = run_scan()
...
if not data_list:
    ...
    for i, item in enumerate(data_list):
        ...
        with st.expander(f"{item['sid']} {item['name']} ({item['price']:.2f}) ➔ {item['sign']}", expanded=True):
            ...
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False}, key=f"v_{item['sid']}_{i}")