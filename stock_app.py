...
# --- 1. 網頁基本設定 ---
st.set_page_config(layout="wide", page_title="金虎南-動態共振監控終極版")
        ...
        try:
            ...
            for idx in range(len(view_df) - 3, -1, -1):
                ...
                if all(w_init['Low'].iloc[j] <= w_init['MA_S'].iloc[j] <= w_init['High'].iloc[j] for j in range(3)):
                    ...
                    if box_height_pct <= 2.5:
                        status_tag = f" ⚡[極度壓縮:{box_height_pct:.1f}%]"
                    elif box_height_pct <= 4.5:
                        status_tag = f" 🎯[黃金壓縮:{box_height_pct:.1f}%]"
                    else:
                        status_tag = f" 📦[寬幅盤整:{box_height_pct:.1f}%]"
...
if "data" not in st.session_state:
    with st.spinner('計算動態共振與壓縮率...'): st.session_state["data"] = run_scan()

data_list = st.session_state.get("data", [])

col_t, col_b = st.columns([8, 2])
with col_t: st.subheader("🐯 金虎南-動態共振監控 (壓縮率自動辨識)")
with col_b:
    if st.button("🔄 刷新"):
...
for i, item in enumerate(data_list):
    ...
    with st.expander(f"{item['sid']} {item['name']} ({item['price']:.2f}) ➔ {item['sign']}", expanded=True):
        ...
        fig.add_trace(go.Candlestick(
            ...
            decreasing_line_color='#004400', decreasing_fillcolor='#004400',
            name="K線"
        ))
        fig.add_trace(go.Scatter(x=df.index, y=df['MA_S'], line=dict(color='#0044BB', width=2.5)))
        ...
        st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False}, key=f"v_final_{item['sid']}_{i}")