...
st.set_page_config(page_title="金虎南均線系統", layout="wide")
st.title("🐯 金虎南：專業級互動掃描系統")

# --- 2. 專業繪圖函數：支援點擊開關均線 + 畫線框框 ---
def draw_kline(df, sid, name, sP, lP):
    ...
    fig = make_subplots(
        ...
        row_width=[0.25, 0.75],
        subplot_titles=(f'📈 {sid} {name} (點擊下方標籤可開關均線)', '📊 成交量能')
    )
    ...
    fig.update_layout(
        ...
        # 圖例放在頂部，這就是讓你「點一下就取消均線」的開關
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
        # 預設啟動畫線模式
        dragmode='drawline',
        # 設定手劃線：白色加粗
        newshape=dict(line_color='White', line_width=4, fillcolor='rgba(255, 255, 255, 0.2)')
    )
...
def run_precise_scan():
    try:
        ...
        st.info(f"🔄 正在同步雲端清單（共 {len(valid_stocks)} 檔）...")
...
if st.session_state.get("results"):
    ...
    for idx, res in enumerate(st.session_state["results"]):
        # 點擊按鈕繪圖
        if st.button(f"📈 畫圖分析: {res['sid']} {res['名稱']}", key=f"btn_{idx}"):
