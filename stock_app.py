...
# --- 工具函數：即時抓取 Yahoo 歷史/當日日線資料 (不使用快取) ---
def get_yahoo_history(sid: str, max_n: int):
    """
    即時抓取 Yahoo 歷史日線資料 (包含最新價格)。
...
def fetch_signals(sid, short_n, long_n):
    try:
        ...
        # 2. 定義時間點資料 (index 0 為最新/當天，1 為昨日，2 為前日)
        ...
        for label, n in ma_list:
            ...
            # 計算均線：T_ma (含當日), Y_ma (歷史當日前一天) 與 B_ma (歷史當日前兩天)
            T_ma = get_ma(cls, n, 0)
