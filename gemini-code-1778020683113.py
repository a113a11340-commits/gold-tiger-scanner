    ...
    for item in all_targets:
        try:
            ...
            for suffix in [".TW", ".TWO"]:
                temp_tk = yf.Ticker(f"{sid}{suffix}")
                h = temp_tk.history(period="1d")
                if not h.empty:
                    tk = temp_tk
            ...
            # 突破判定
            is_break_above = (prev_p <= prev_ma * 1.002) and (curr_p > curr_ma)
            is_break_below = (prev_p >= prev_ma * 0.998) and (curr_p < curr_ma)
            ...
            if is_break_above or is_break_below:
                ...
                item.update({
                    "df": hist,
                    "sign": f"{'🚀突破' if is_break_above else '🚨跌破'} {s_day}MA | 即時:{curr_p:.2f} | MA:{curr_ma:.2f}",
                })
                results.append(item)
...
if not data_list:
    ...
    for i, item in enumerate(data_list):
        ...
        with st.expander(f"{item['sid']} {item['name']} ➔ {item['sign']}", expanded=True):
            ...
            fig.update_layout(
                height=150, # <--- 已縮小高度
                showlegend=False, xaxis_rangeslider_visible=False, margin=dict(l=5, r=5, t=5, b=5),
                xaxis=dict(type='category', range=[t_len-42, t_len-0.5], showticklabels=False, fixedrange=True),
                yaxis=dict(side='right', fixedrange=True), hovermode=False
            )
